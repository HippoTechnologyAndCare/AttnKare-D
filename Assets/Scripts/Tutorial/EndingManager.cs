    ...
    public class EndingManager : MonoBehaviour
{
    ...
    IEnumerator StartEnding()
    {
        ...
        */
        if (DataManager.GetInstance().userInfo.Grade != null)
        {
            if (UserInfo_API.GetInstance().playerInfo.grade<4) //저학년
            {
                StartCoroutine(Ghost.GetComponent<Actor>().ghostSpeak("<size=0.045>오늘 책가방 챙기기,\n공 옮기기,\n숫자맞추기,\n방 청소하기를 해봤는데", audioIndex = 1, 7.0f));
            }
