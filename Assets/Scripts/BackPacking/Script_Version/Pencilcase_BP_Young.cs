...
public class Pencilcase_BP_Young : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        arr_Pos = new Vector3[] { new Vector3(-1e-05f, 0.000851f, 4.4e-05f), //지우개 자리
                                  new Vector3(0.000316f, m_fY, m_fZ), //아래서부터는 연필, 펜 자리인데 들어오는 순서대로 set 하기
                                  new Vector3(0.00011f, m_fY, m_fZ),
                                  new Vector3(-0.000107f, m_fY, m_fZ),
                                  new Vector3(-0.000326f, m_fY, m_fZ),
        };
    ...
    void SetPosition(bool bEraser)
    {
        ...
        m_nStg1++;
        if(m_nStg1 == 5) StartCoroutine(AllDone());//wait for seconds end activate pcap
    }
    void SetPencil()
    {
        if (m_nPncilIndex < 4)
