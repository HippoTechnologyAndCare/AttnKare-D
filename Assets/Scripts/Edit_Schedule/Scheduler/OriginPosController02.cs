...
namespace Scheduler
{
    public class OriginPosController02 : MonoBehaviour
    {
        ...
        public void CardDestroyer(GameObject cardB)
        {
            if (storedCard == null) //같은 계열의 카드일때 
            {
                ...
                cardB.GetComponent<PlanCubeController2>().activeSlot = null;
            ...
            storedCard.GetComponent<PlanCubeController2>().activeSlot = null;
            Debug.Log((storedCard.name));
            Debug.Log(storedCard.GetComponent<PlanCubeController2>().activeSlot == null);
        }
        
        private void OnTriggerStay(Collider other)
        {
            if (!other.CompareTag("PLAN")) return;
            other.GetComponent<PlanCubeController2>().cardState = PlanCubeController2.CardState.Idle;
            otherName = other.name.Replace("(Clone)", "");

            if (name == otherName && !isStored)
            {                    
                other.GetComponent<PlanCubeController2>().isHomeTW = true;
                isStored = true;
        ...
        private void OnTriggerExit(Collider other)
        {
            ...
            other.GetComponent<PlanCubeController2>().isHomeTW = false;
            isStored = false;
            storedCard = null;
        }
